...
**Minecraft Transit Railway (MTR) 客户端性能优化模组**
...
- 阈值：`liftMaxRenderDistance`（默认 192 格）

### 数据同步频率节流
MTR 4 的客户端通过 HTTP 轮询 Transport Simulation Core 获取列车位置和状态数据。玩家远离所有列车和电梯时降低轮询频率，减少 CPU 开销和 HTTP 请求。

- 全速同步距离：`fullSyncDistance`（默认 64 格）
- 降频周期：`reducedSyncInterval`（默认每 20 tick 一次）
- 超出 `noSyncDistance`（256 格）完全停止同步
...
1. 确保已安装 **Fabric Loader 0.19.3+** 和 **MTR 4.0.0+**
2. 从 Releases 下载 `mmtr-*.jar` 或自行构建
...
首次运行后自动生成 `config/mmtr.json`，修改后无需重启即可生效（后续可加 `/mmtr reload` 命令）。

完整参数说明：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `vehicleFullRenderDistance` | 64 | 列车在此距离内渲染全部细节（外壳+内饰+灯光） |
| `vehicleMaxRenderDistance` | 256 | 列车超过此距离完全停止渲染 |
| `blockEntityMaxRenderDistance` | 128 | PSD/信号/PIDS/站名牌最大渲染距离 |
| `liftMaxRenderDistance` | 192 | 电梯最大渲染距离 |
| `fullSyncDistance` | 64 | 全速数据同步范围（格） |
| `reducedSyncDistance` | 128 | 降频同步范围（格） |
| `reducedSyncInterval` | 20 | 降频同步间隔（tick） |
| `noSyncDistance` | 256 | 超过此距离停止所有同步 |
...
| `debugRenderDistance` | false | 调试模式 |
